...
- **Version / Case ID tracking** — track fixes like `CSCvi23216` over time.

---

## Running locally in VS Code

The app is fully static — **no build step required**. Because it loads category templates and uses local library files, you should serve it over HTTP (don’t just double‑click `index.html`).
...
├── data/templates/         # One sample CSV per category
...
| `CaseID`   | No       | text                     | Case / bug reference for traceability (e.g. `CSCvi23216`). |
...
- **Auto status**: if `Status` is blank but both `Value` and `Target` are present, status is derived -- >=100% of target = Green, >=80% = Amber, else Red.
- **Unknown columns** are ignored (with a non‑blocking warning); **blank rows** are skipped.

### Example
```csv
Date,Category,Metric,Value,Target,Status,Version,CaseID,Owner,Notes
2025-03-31,Cases,P1 Critical Cases,0,0,green,17.9.5,CSCvi23216,Tier 3,Defect resolved in 17.9.5
```

A ready‑to‑edit template for every category lives in **`data/templates/`**, and you can download each one from the **Settings** tab inside the app.
...
No framework, no bundler — just open and run.
